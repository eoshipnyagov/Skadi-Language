...
- Emit C file:
  - `cargo run -- --input program.scadi --emit-c out.c`

- Build executable in one command (requires `gcc` or `clang` in PATH):
  - `cargo run -- --input program.scadi --emit-exe out.exe`

- Print generated C to stdout:
  - `cargo run -- --input program.scadi --print-c`
