...
## 10. Style Profile v1 (каноничный стиль)

Цель: код читается без напряжения, без визуального шума.

- Циклы:
  - предпочтительно `iterate <collection> as <item>`;
  - `for <item> in <collection>` оставлен для совместимости.
- Имена типов:
  - фиксированные числовые: `i8/i16/i32/i64`, `u8/u16/u32/u64`, `f32/f64`;
  - человекочитаемые: `Int`, `Float`, `Bool`, `Char`, `Text`, `Path`, `List`, `Vec2`, `Vec3`, `Vec4`.
- Разбор CLI-флагов:
  - предпочтительно через `when`, а не длинные цепочки `if`.
- Стиль вывода:
  - для чисел и bool: `output("label:")` затем `output(value)`;
  - для текста: `output(concat("label: ", text))`.
- Ошибки:
  - для рискованных операций использовать `on error`;
  - коды ошибок писать предметно (`ZeroDivision`, `BadInput`, ...).
