...
- сравнение;
- логические `and`, `or`, `xor`, prefix `not`;
- prefix `-` для числового значения;
- группировка через `(...)`;
- function calls;
- method calls;
- field access;
...
new Int quotient = total div 2
new Int remainder = total mod 2
...
- typed params;
- канонический typed return через `returns`;
- вызов функции внутри выражения;
- проверка количества и типов аргументов.

Legacy-форма `fn name(...) Type` пока принимается для scalar-типов с предупреждением.
Для возвращаемых структур `returns` обязателен.
...
Видимость задаётся явно там, где она отличается от публичной по умолчанию:
...
- `local fn`, `local struct` и `local label` видны только в своём файле;
- `hide`-поле доступно только методам той же структуры;
- shadowing локальных имён запрещён.

## 12.1 Импорты и квалифицированные имена

Текущий стабильный контракт использует относительные path-imports:

```skadi
import "./math_utils.skd"

new Int direct = add(2, 3)
new Int qualified = math_utils.add(2, 3)
```

- имя модуля равно имени файла без `.skd`;
- напрямую импортированные публичные функции, структуры и labels доступны без квалификации;
- `module.symbol` разрешает коллизии и явно указывает источник;
- транзитивные импорты не переэкспортируются;
- циклы, отсутствующие файлы и коллизии получают диагностики `SC-MOD-*`;
- `import module_name`, aliases и re-exports пока не реализованы.
...
## 18.3 Experimental `Time` / `Duration` MVP
...
new Time deadline = started_at + 1s
new Bool completed = measured >= budget
```

Текущий контракт:

- `Time` и `Duration` являются nominal value-safe types;
- literals `ms`, `s`, `min` принимают только целую слитную запись;
- внутреннее представление - signed `i64` nanoseconds;
- `Time` использует monotonic clock и не является датой или Unix timestamp;
- поддерживаются `now()`, `elapsed(Time)`, `sleep(Duration)`, `delay(Duration)`;
- `Time - Time` возвращает `Duration`;
- `Time +/- Duration` возвращает `Time`;
- `Duration +/- Duration` возвращает `Duration`;
- сравнения разрешены только между одинаковыми nominal types;
