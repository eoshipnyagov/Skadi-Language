...
- `examples/memory/positive/`
  - корректные frontend-примеры;
  - они должны проходить parser и semantic;
  - build до backend сейчас обязан останавливаться на `SC-CG-201`, потому что lowering memory model ещё не реализован.
...
### 3.3. Возврат `struct` со списком из внешней `Memory`
...
- explicit field init остаётся читаемым даже в маленьком примере.
