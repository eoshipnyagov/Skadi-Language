...
- `examples/memory/large/`
  - более объёмные canonical examples, где memory surface показан как связный рабочий сценарий;
  - рядом с ними лежат реальные входные данные или generator-скрипты, чтобы пример можно было сразу скормить компилятору и прогнать.
...
### 3.5. Большой canonical example: log analyzer

Файлы:

- `examples/memory/large/01_log_analyzer.skd`
- `examples/memory/large/data/sample_service.log`

Что показывает:

- локальную `scratch_memory` для временного preview;
- внешнюю `assets_memory` для region-owned данных, которые живут дольше одного блока;
- `Text`, `Text List`, helper-функции, циклы и явные self-contained имена;
- сценарий, который похож на реальную утилиту, а не на синтетический micro-test.

Что стоит смотреть в первую очередь:

- `preview_log(...)` как локальную scratch-memory ветку;
