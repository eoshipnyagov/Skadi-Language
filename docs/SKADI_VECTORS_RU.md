...
`Vec2`, `Vec3` и `Vec4` - встроенные value-типы с компонентами `f64`. Размерность
...
поддерживаются. Деление на нулевой scalar следует обычному поведению C `double`.
...
Каждый тип lower'ится в небольшой C `struct` из `double`, а операции - в
