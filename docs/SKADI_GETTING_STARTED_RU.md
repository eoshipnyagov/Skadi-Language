...
## Первый одиночный файл

Для небольшого примера или утилиты manifest не нужен:

```powershell
skadi-cli quick-run hello.skd
```

`quick-run` использует временную native-сборку и удаляет артефакты после
завершения. Cargo установленному пользователю не нужен. Аргументы программы
передаются после `--`.
