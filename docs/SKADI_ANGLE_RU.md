...
`Angle` - nominal-тип угла. Он хранится как `f64` radians, но не является alias
...
должно помещаться в finite `f64`; `NaN` и infinity не создаются literal parser.
...
получает отдельной runtime-политики в этом MVP и следует поведению C `double`.
...
`sin(Angle)` и `cos(Angle)` возвращают `Float`. Для совместимости с math core
`v1.1` они временно также принимают numeric raw radians, но canonical `v1.2`
код использует `Angle`. `rad_to_deg(Float)` больше не является допустимой
канонической формой.
...
- нет `normalize_angle`, `tan`, `asin`, `acos`, `atan`;
