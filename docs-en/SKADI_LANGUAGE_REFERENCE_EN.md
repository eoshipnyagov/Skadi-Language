# Complete Language Reference

Use the [quick reference](language-quick-reference.en.md) for a single complete
inventory. The detailed reference is split by topic:

## Core language

- [Types, literals, and expressions](language-basics.en.md)
- [Control flow](control-flow.en.md)
- [Functions and errors](functions-errors.en.md)
- [Struct, Text, and List](data-model.en.md)
- [Files, imports, and visibility](modules.en.md)
- [C ABI and native C](c-abi.en.md)
- [I/O and filesystem](io-files.en.md)
- [Math](math.en.md)

## Experimental `v1.2` systems surface

- [Memory](memory.en.md)
- [Ownership and resource transfer](ownership.en.md)
- [Task and Channel](concurrency.en.md)
- [Time and Duration](time-duration.en.md)
- [ByteSize](byte-size.en.md)
- [Angle](angle.en.md)
- [Vectors](vectors.en.md)
- [Platform Int and bit operations](bits.en.md)

## Practice and platforms

- [Recommended practices](practices.en.md)
- [Embedded status](embedded.en.md)
- [Canvas and Visual Core](canvas.en.md)
- [Syntax status](syntax-status.en.md)
