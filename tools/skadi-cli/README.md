...
типами, `external struct` для by-value C-layout, call-scoped `Buffer(T)` и
opaque owning `external resource` handles с явным `view`/`edit`/`move`.
Raw pointer ABI по-прежнему намеренно не поддерживается.
